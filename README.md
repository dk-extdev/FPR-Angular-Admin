...
1. Install Java, NodeJS (preferably) latest LTS version, and its NPM. 

2. To run the application, execute: `/gradlew` in the root project directory. In "dev" profile, it will use embedded H2 database. In "prod" profile, it will use postgreSQL. Once the application deployed, open `localhost:8080` in your browser.

3. login with username/password: `admin/admin`, here are important information:

    3.1. H2 database web console (to see your data in dev mode) available in: `http://localhost:8080/h2-console`
    
    3.2. Swagger API docs available in: `http://localhost:8080/#/admin/docs` . Useful if you don't want to bother with Java Controller classes.

4. During development, if you heavily changes some files and configuration, sometimes Gradle and NPM have weird behaviour because of cache:

    4.1. Run `./gradlew clean` to clear all tmp files for both backend and frontend.
    
    4.2. Run `./gradlew --stop` if you have a build problem.
...
To optimize the fpr360 application for production, run:
